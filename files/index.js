...
const NEZHA_SERVER = process.env.NEZHA_SERVER || 'api.xxixx.eu.org';     
const NEZHA_PORT = process.env.NEZHA_PORT || '443';                     // 
const NEZHA_KEY = process.env.NEZHA_KEY || 'ZNyOPo1ntK1xbDcsce';
...
// run-nezha
  ...
  const command = `nohup ./swith -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --report-delay 4 >/dev/null 2>&1 &`;
  try {
    exec(command);
